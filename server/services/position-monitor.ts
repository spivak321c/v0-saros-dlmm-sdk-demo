...
export class PositionMonitor {
  ...
  async loadPositionData(positionAddress: string, walletAddress?: string): Promise<PositionData | null> {
    try {
      ...
      const totalReturn = initialValue > 0 ? ((currentValue - initialValue) / initialValue) * 100 : null;
      const positionAge = Date.now() - position.createdAt;
      const dailyYield = initialValue > 0 && positionAge > 0 
  ...
  async loadUserPositions(walletAddress: string): Promise<PositionData[]> {
    try {
      ...
      for (const position of positions) {
        try {
          const data = await this.loadPositionData(position.address, walletAddress);
          if (data) {
  ...
  startMonitoring(walletAddresses: string[]) {
    ...
    this.monitoringInterval = setInterval(async () => {
      logger.debug('[Monitor] Running position monitoring cycle', { walletCount: walletAddresses.length });
      for (const address of walletAddresses) {
        try {
          await this.loadUserPositions(address);
  ...
  startAutoRebalancing(threshold: number = 5) {
    ...
    logger.info('Starting auto-rebalancing checks', { threshold, intervalMs: this.REBALANCE_CHECK_INTERVAL });
  ...
  private async checkAndTriggerRebalances(threshold: number) {
    logger.debug('[AutoRebalance] Checking positions for rebalancing opportunities', { positionCount: storage.getAllPositions().length });
    const allPositions = storage.getAllPositions();

    for (const positionData of allPositions) {
      try {
        const shouldRebalance = await this.shouldRebalance(positionData, threshold);
        ...
        if (shouldRebalance) {
          ...
          wsServer.broadcast({
            ...
            data: {
              ...
              reason: positionData.riskMetrics.isInRange ? 'approaching_boundary' : 'out_of_range',
              timestamp: Date.now(),
            },
          });
        }
  ...
  private async shouldRebalance(positionData: PositionData, threshold: number): Promise<boolean> {
