...
- **Bot**: Telegraf
- **Frontend**: Next.js 14, React, Chart.js
- **Data Fetching**: SWR
...
\`\`\`typescript
import { DLMMPool, getUserPositions, addLiquidityIntoPosition } from '@saros-finance/dlmm-sdk';

// Load pool
const pool = await DLMMPool.load(connection, poolAddress);

// Get user positions
const positions = await getUserPositions(connection, wallet.publicKey);

// Calculate volatility from bins
const binData = await pool.getBinArray();
const volatility = calculateVolatility(binData);

// Rebalance position
await removeMultipleLiquidity(connection, position);
await addLiquidityIntoPosition(connection, newPosition, {
...
const rangeMultiplier = 1 + (stdDev / mean);
const newRange = baseRange * rangeMultiplier;
\`\`\`
...
Built with ❤️ for Saros Finance Hackathon
